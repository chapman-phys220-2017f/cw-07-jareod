...
"""Code creates an 'n x n' matrix such that, when mulitplied by a vector,
returns the approximate derivative of the vector given by the forward, symmetric, and
backward differences appropriately."""
...
def f(a,b,n):
    """Function to return an array of the square of the input (a float)"""
    t = np.linspace(a,b,n)
    return t**2

def s(a,b,n):
    """Function to return an array of the sine of the input (float)"""
    t = np.linspace(a,b,n)
...
def g(a,b,n):
    """Function to return an array of the gaussian function"""
    def gauss(x):
        """Local function to return the Gaussian function dependent on the input float"""
        c = 1/(np.sqrt(2*np.pi))
...
def plot_function(a,b,n,f,string):
    ...
    Plots a function on the domain specified by a and b, with n points, for
    a function f, labeled with a string describing the function"""
    t = np.linspace(a,b,n)
